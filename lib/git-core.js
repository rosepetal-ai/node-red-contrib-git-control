...
module.exports = function(RED) {
    ...
    // (not merge) and is scanned by simple-git's safety checks (the scanner
    // rejects names like EDITOR, PAGER, SSH_ASKPASS, plus a long list of GIT_*).
    // An allowlist is the durable design: only forward what git+ssh actually
    // need, and anything new that simple-git decides to block in the future is
    // already excluded. Adding to FORWARDED_ENV is explicit and reviewable.
    const FORWARDED_ENV = new Set([
        // Executable lookup + user home (resolves git, ssh, ~/.gitconfig, ~/.ssh)
        'PATH', 'HOME', 'USER', 'LOGNAME',
        // Locale (so git's plumbing output and ssh prompts are consistent)
        'LANG', 'LC_ALL', 'LC_CTYPE', 'LC_MESSAGES',
        // Temp dirs (git uses them for index lock / merge work)
        'TMPDIR', 'TMP', 'TEMP',
        // XDG paths (newer Linux puts ~/.config/git there)
        'XDG_CONFIG_HOME', 'XDG_CACHE_HOME', 'XDG_DATA_HOME',
        // Corporate proxy plumbing (libcurl honors these)
        'HTTP_PROXY', 'HTTPS_PROXY', 'NO_PROXY', 'ALL_PROXY',
        'http_proxy', 'https_proxy', 'no_proxy', 'all_proxy',
        // ssh-agent socket (harmless if unused; needed if a user runs an agent)
        'SSH_AUTH_SOCK'
    ]);
    ...
    function getRemoteGit(repoPath, auth, extraConfig = []) {
        return getGit(repoPath, extraConfig, {
    ...
    // Read the flow file name a Node-RED project declares in its manifest.
    function projectFlowFile(resolvedRepoPath) {
        try {
            const pkg = JSON.parse(fs.readFileSync(path.join(resolvedRepoPath, 'package.json'), 'utf8'));
            const nr = pkg && pkg['node-red'];
            const flowFile = nr && ((nr.settings && nr.settings.flowFile) || nr.flowFile);
            return typeof flowFile === 'string' && flowFile ? flowFile : null;
        } catch (error) {
            return null; // not a project repo, or no/invalid manifest
        }
    }

    // Resolve Node-RED's flow file path relative to the repo root, so flow-aware
    // operations can read it at any ref. Prefers the project manifest (projects
    // mode), then RED.settings.flowFile, and finally "flows.json" (the Node-RED
    // project default) when neither maps into the repo.
    function getFlowFileRelPath(repoPath) {
        const resolved = resolveRepoPath(repoPath);

        const fromManifest = projectFlowFile(resolved);
    ...
    // Resolve the repo's git directory.
    function getGitDir(repoPath) {
        ...
        try {
            ...
            if (m) return path.resolve(resolved, m[1].trim());
        } catch (error) { /* fall through to the default */ }
        return dotGit;
    }

    // Detect an interrupted operation that left the repo mid-way (typically after a conflict).
    function getMergeState(repoPath) {
        const gitDir = getGitDir(repoPath);
        const has = marker => { try { return fs.existsSync(path.join(gitDir, marker)); } catch (e) { return false; } };
        let kind = null;
    ...
    function formatGitError(error, operation) {
        ...
        const errorPatterns = [
            ...
            {
                pattern: /needs merge|is unmerged|unmerged files|path '.*' is unmerged/i,
                message: 'File has unresolved merge conflicts',
                suggestion: 'This file is mid-conflict, so it can\'t be discarded directly. Resolve the conflict and commit, or use "Abort" on the conflict banner to undo the operation.'
            },
            {
            ...
            {
                pattern: /not fully merged|not yet merged/i,
                message: 'Branch is not fully merged',
                suggestion: 'This branch has commits that are not merged anywhere else. Confirm a force-delete to remove it anyway (those commits will be lost).'
            },
            {
    ...
    return {
        ...
        showFileAtRef,
        getFlowFileRelPath,
        // conflict / in-progress operation state
        getMergeState,
        // errors
        formatGitError,
        formatRemoteError
