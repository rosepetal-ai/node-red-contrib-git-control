...
module.exports = function(RED) {
    ...
    // Helper function to get git user settings from Node-RED user settings
    // Falls back to null if not configured, allowing git config fallback
    function getGitUserSettings(req) {
        try {
            // Get username from request (authenticated) or use anonymous user "_"
            const username = req?.user?.username || "_";
            const userSettings = RED.settings.getUserSettings(username) || {};
            const gitSettings = userSettings.git || {};

            if (gitSettings.user?.name || gitSettings.user?.email) {
                return {
    ...
    RED.httpAdmin.get("/rosepetal-git/project-info",
        ...
        async function(req, res) {
            try {
                ...
                // Get user settings: prefer Node-RED settings, fallback to git config
                const nodeRedGitSettings = getGitUserSettings(req);
                ...
                if (nodeRedGitSettings) {
                    ...
                    // Fallback to git repository config
