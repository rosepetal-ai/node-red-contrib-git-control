...
module.exports = function(RED) {
    ...
    RED.httpAdmin.post("/rosepetal-git/pull", write, async function(req, res) {
        try {
            res.json(await svc.pull({ ...(req.body || {}), user: req.user }));
        } catch (error) {
    ...
    RED.httpAdmin.post("/rosepetal-git/merge", write, async function(req, res) {
        try {
            res.json(await svc.merge({ ...(req.body || {}), user: req.user }));
        } catch (error) {
    ...
    // POST /rosepetal-git/abort - Abort an in-progress merge/cherry-pick/revert/rebase
    RED.httpAdmin.post("/rosepetal-git/abort", write, async function(req, res) {
        try {
            res.json(await svc.abortOperation(req.body || {}));
        } catch (error) {
            fail(res, error, 'abort', 'git');
        }
    });

    // POST /rosepetal-git/merge/preview - Preview flow-aware resolution of an
    // in-progress merge (auto-merged count, per-node/per-unit conflicts).
    RED.httpAdmin.post("/rosepetal-git/merge/preview", read, async function(req, res) {
        try {
            res.json(await svc.previewMergeResolution(req.body || {}));
        } catch (error) {
            fail(res, error, 'merge-preview', 'git');
        }
    });

    // POST /rosepetal-git/merge/resolve - Apply user choices and finalize the merge commit.
    RED.httpAdmin.post("/rosepetal-git/merge/resolve", write, async function(req, res) {
        try {
            res.json(await svc.resolveConflict({ ...(req.body || {}), user: req.user }));
        } catch (error) {
            fail(res, error, 'merge-resolve', 'git');
        }
    });
    ...
    // POST /rosepetal-git/commit-branches - Branches that contain a commit
    RED.httpAdmin.post("/rosepetal-git/commit-branches", read, async function(req, res) {
        try {
            res.json(await svc.getCommitBranches(req.body || {}));
        } catch (error) {
            fail(res, error, 'commit branches', 'git');
        }
    });
