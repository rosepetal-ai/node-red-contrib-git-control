...
module.exports = function(RED, core) {
    ...
    async function merge({ repoPath, ref, noFastForward = false } = {}) {
        ...
        return { success: true, operation: 'merge', ref: source, result: (result || '').trim() };
    }
    ...
    async function revertCommit({ repoPath, commitRef, noCommit = false, user } = {}) {
        ...
        args.push(ref);
        const result = await git.raw(args);
        return { success: true, operation: 'revert', commit: ref, noCommit, result: (result || '').trim() };
    }
    ...
    async function cherryPick({ repoPath, commitRef, noCommit = false, user } = {}) {
        ...
        args.push(ref);
        const result = await git.raw(args);
        return { success: true, operation: 'cherry-pick', commit: ref, noCommit, result: (result || '').trim() };
    }
