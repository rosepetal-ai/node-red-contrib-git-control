...
module.exports = function(RED, core) {
    ...
    // Return a git instance whose commits carry whatever identity we could
    // resolve. When nothing (or only one field) is configured we hand the rest
    // to git's own auto-detection, like Node-RED's projects feature does; if
    // that fails too, git's real error surfaces through the error formatter.
    ...
    async function forcePush({ repoPath } = {}) {
        ...
        const currentBranch = core.getBranchNameFromStatus(status);
        if (!currentBranch) {
            throw new Error('Cannot force-push while in detached HEAD state');
        }
        ...
        if (status.tracking && status.tracking.includes('/')) {
            ...
            const remoteName = status.tracking.slice(0, slash);
            const remoteBranch = status.tracking.slice(slash + 1);
            await remote.raw(['push', '--force-with-lease', remoteName, `HEAD:${remoteBranch}`]);
        } else {
            throw new Error('Cannot force-push: current branch has no upstream configured');
        }
